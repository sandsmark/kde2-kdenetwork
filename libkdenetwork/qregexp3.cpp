/****************************************************************************
** $Id: qregexp3.cpp 89864 2001-04-02 00:42:30Z gebauer $
**
** Implementation of QRegExp3 class
**
** Created : 950126
**
...
/*!
  ...
  \brief The QRegExp3 class provides pattern matching using regular expressions.
  ...
  \ingroup shared
    ...
    documentation. QRegExp3 is modelled on Perl's regexp engine and fully
    supports Unicode. QRegExp3 may also be used in the weaker 'wildcard'
    ...
    supported by QRegExp3, see
    <a href="#characters-and-abbreviations-for-sets-of-characters">
    characters and abbreviations for sets of characters</a>.

    In QRegExp3, apart from within character classes, <tt>^</tt> always
    ...
    necessary. The same applies to <tt>$</tt> which in QRegExp3 always
    signifies the end of the string.

    QRegExp3's quantifiers are the same as Perl's greedy quantifiers.
    ...
    In QRegExp3 <b>.</b> matches any character, therefore all QRegExp
    regexps have the equivalent of Perl's <tt>/s</tt> option. QRegExp3
    ...
    Because QRegExp3 is string oriented there are no \A, \Z or \z
    assertions. The \G assertion is not supported but can be emulated in
    a loop.

    Perl's $& is cap(0) or capturedTexts()[0]. There are no QRegExp3
    ...
    \code
    QRegExp3 rx( "^\\d\\d?$" );	// Match integers 0 to 99
    ...
    \code
    QRegExp3 rx( "^\\S+$" );	// Match strings which have no whitespace
    ...
    \code
    QRegExp3 rx( "\\b(mail|letter|correspondence)\\b" );
    ...
    \code
    QRegExp3 rx( "&(?!amp;)" );	    // Match ampersands but not &amp;
    ...
    Here we've passed the QRegExp3 to QString's replace() function to
    replace the matched text with new text.

    \code
    QString str = "One Eric another Eirik, and an Ericsson. How many Eiriks, Eric?";
    QRegExp3 rx( "\\b(Eric|Eirik)\\b" );	// Match Eric or Eirik
    ...
    \code
    QRegExp3 rx( "*.html" );	// Invalid regexp: * doesn't quantify anything
    ...
    QRegExp3 can match case insensitively using setCaseSensitive(), and
    can use non-greedy matching, see setMinimal(). By default QRegExp3
  ...
  \sa QRegExp3Validator QString QStringList
...
/*
  The class QRegExp3Engine encapsulates a modified nondeterministic finite
  automaton (NFA).
*/
class QRegExp3Engine : public QShared
{
public:
	...
	*/
    ...
    QRegExp3Engine( bool caseSensitive ) { setup( caseSensitive ); }
    QRegExp3Engine( const QString& rx, bool caseSensitive );
#ifndef QT_NO_REGEXP_OPTIM
    ~QRegExp3Engine();
#endif
...
private:
    ...
    */
    struct Lookahead
    {
	QRegExp3Engine *eng; // NFA representing the embedded regular expression
	bool neg; // negative lookahead?

	Lookahead( QRegExp3Engine *eng0, bool neg0 )
	    : eng( eng0 ), neg( neg0 ) { }
...
#ifndef QT_NO_REGEXP_LOOKAHEAD
    int addLookahead( QRegExp3Engine *eng, bool negative );
#endif
    ...
    */
	...
	QRegExp3Engine *eng; // the automaton under construction
...
QRegExp3Engine::QRegExp3Engine( const QString& rx, bool caseSensitive )
#ifndef QT_NO_REGEXP_BACKREF
    : mmSleeping( 101 )
#endif
{
...
*/
QArray<int> QRegExp3Engine::match( const QString& str, int pos, bool minimal,
				  bool oneTest )
{
    mmStr = &str;
    mmIn = str.unicode();
...
int QRegExp3Engine::createState( QChar ch )
{
    return setupState( ch.unicode() );
}

int QRegExp3Engine::createState( const CharClass& cc )
{
#ifndef QT_NO_REGEXP_CCLASS
    int n = cl.size();
    cl.resize( n + 1 );
...
#ifndef QT_NO_REGEXP_BACKREF
int QRegExp3Engine::createState( int bref )
{
    if ( bref > nbrefs ) {
	nbrefs = bref;
...
void QRegExp3Engine::addCatTransitions( const QArray<int>& from,
				       const QArray<int>& to )
{
...
#ifndef QT_NO_REGEXP_CAPTURE
void QRegExp3Engine::addPlusTransitions( const QArray<int>& from,
...
*/
int QRegExp3Engine::anchorAlternation( int a, int b )
{
...
*/
int QRegExp3Engine::anchorConcatenation( int a, int b )
{
...
*/
void QRegExp3Engine::addAnchors( int from, int to, int a )
{
    State *st = s[from];
    if ( st->anchors == 0 )
...
void QRegExp3Engine::setupGoodStringHeuristic( int earlyStart, int lateStart,
					      const QString& str )
{
...
void QRegExp3Engine::setupBadCharHeuristic( int minLen,
...
*/
void QRegExp3Engine::heuristicallyChooseHeuristic()
{
    int i;
...
#if defined(QT_DEBUG)
void QRegExp3Engine::dump() const
{
    int i, j;
...
void QRegExp3Engine::setup( bool caseSensitive )
{
#ifndef QT_NO_REGEXP_OPTIM
    if ( engCount++ == 0 ) {
...
int QRegExp3Engine::setupState( int match )
{
...
*/
int QRegExp3Engine::startAtom( bool capture )
{
...
*/
int QRegExp3Engine::addLookahead( QRegExp3Engine *eng, bool negative )
{
    int n = ahead.size();
    if ( n == MaxLookaheads ) {
	yyError = TRUE;
...
*/
bool QRegExp3Engine::isBetterCapture( const int *begin1, const int *end1,
...
/*
  ...
  otherwise FALSE.
*/
bool QRegExp3Engine::testAnchor( int i, int a, const int *capBegin )
{
    int j;
...
bool QRegExp3Engine::goodStringMatch()
{
...
bool QRegExp3Engine::badCharMatch()
{
    int slideHead = 0;
    int slideNext = 0;
    int i;
...
#else
bool QRegExp3Engine::bruteMatch()
{
    while ( mmPos <= mmLen ) {
	if ( matchHere() )
	    return TRUE;
...
QRegExp3Engine::CharClass::CharClass()
    : c( 0 ), n( FALSE )
#ifndef QT_NO_REGEXP_OPTIM
      , occ1( *noOccurrences )
#endif
...
QRegExp3Engine::CharClass& QRegExp3Engine::CharClass::operator=(
	const CharClass& cc )
{
    c = cc.c;
    r = cc.r.copy();
...
void QRegExp3Engine::CharClass::clear()
{
    c = 0;
    r.resize( 0 );
    n = FALSE;
...
void QRegExp3Engine::CharClass::setNegative( bool negative )
{
    n = negative;
#ifndef QT_NO_REGEXP_OPTIM
    occ1 = *firstOccurrenceAtZero;
...
void QRegExp3Engine::CharClass::addCategories( int cats )
{
    c |= cats;
#ifndef QT_NO_REGEXP_OPTIM
    occ1 = *firstOccurrenceAtZero;
...
void QRegExp3Engine::CharClass::addRange( ushort from, ushort to )
{
    if ( from > to )
	qSwap( from, to );
    int n = r.size();
...
bool QRegExp3Engine::CharClass::in( QChar ch ) const
{
#ifndef QT_NO_REGEXP_OPTIM
...
#if defined(QT_DEBUG)
void QRegExp3Engine::CharClass::dump() const
{
    int i;
...
QRegExp3Engine::Box::Box( QRegExp3Engine *engine )
...
QRegExp3Engine::Box& QRegExp3Engine::Box::operator=( const Box& b )
{
    eng = b.eng;
    ls = b.ls;
    rs = b.rs;
...
void QRegExp3Engine::Box::set( QChar ch )
{
    ls.resize( 1 );
...
void QRegExp3Engine::Box::set( const CharClass& cc )
{
    ls.resize( 1 );
...
#ifndef QT_NO_REGEXP_BACKREF
void QRegExp3Engine::Box::set( int bref )
{
    ls.resize( 1 );
...
void QRegExp3Engine::Box::cat( const Box& b )
{
...
void QRegExp3Engine::Box::orx( const Box& b )
{
    mergeInto( &ls, b.ls );
    mergeInto( &lanchors, b.lanchors );
    mergeInto( &rs, b.rs );
...
void QRegExp3Engine::Box::plus( int atom )
{
#ifndef QT_NO_REGEXP_CAPTURE
...
void QRegExp3Engine::Box::catAnchor( int a )
{
    if ( a != 0 ) {
	for ( int i = 0; i < (int) rs.size(); i++ ) {
...
#ifndef QT_NO_REGEXP_OPTIM
void QRegExp3Engine::Box::setupHeuristics()
{
...
#if defined(QT_DEBUG)
void QRegExp3Engine::Box::dump() const
{
    int i;
...
void QRegExp3Engine::Box::addAnchorsToEngine( const Box& to ) const
{
...
int QRegExp3Engine::getChar()
{
    return ( yyPos == yyLen ) ? EOS : yyIn[yyPos++].unicode();
}

int QRegExp3Engine::getEscape()
{
#ifndef QT_NO_REGEXP_ESCAPE
...
#ifndef QT_NO_REGEXP_INTERVAL
int QRegExp3Engine::getRep( int def )
{
    if ( yyCh >= '0' && yyCh <= '9' ) {
	int rep = 0;
	do {
...
#ifndef QT_NO_REGEXP_LOOKAHEAD
void QRegExp3Engine::skipChars( int n )
{
    if ( n > 0 ) {
	yyPos += n - 1;
	yyCh = getChar();
...
void QRegExp3Engine::startTokenizer( const QChar *rx, int len )
{
    yyIn = rx;
    yyPos0 = 0;
    yyPos = 0;
...
int QRegExp3Engine::parse( const QChar *pattern, int len )
{
    valid = TRUE;
...
void QRegExp3Engine::parseAtom( Box *box )
{
#ifndef QT_NO_REGEXP_LOOKAHEAD
    QRegExp3Engine *eng = 0;
    bool neg;
    int len;
#endif
...
void QRegExp3Engine::parseFactor( Box *box )
{
#ifndef QT_NO_REGEXP_CAPTURE
...
void QRegExp3Engine::parseTerm( Box *box )
{
#ifndef QT_NO_REGEXP_OPTIM
...
void QRegExp3Engine::parseExpression( Box *box )
{
    parseTerm( box );
    while ( yyTok == Tok_Bar ) {
	Box rightBox( this );
...
/*
  The class QRegExp3Private contains the private data of a regular expression
  other than the automaton.  It makes it possible for many QRegExp3 objects to
  use the same QRegExp3Engine object with different QRegExpPrivate objects.
*/
struct QRegExp3Private
{
...
#ifndef QT_NO_REGEXP_CAPTURE
    QString t; // last string passed to QRegExp3::search() or searchRev()
    QStringList capturedCache; // what QRegExp3::capturedTexts() returned last
#endif
    QArray<int> captured; // what QRegExp3Engine::search() returned last

    QRegExp3Private() { captured.fill( -1, 2 ); }
};

#ifndef QT_NO_REGEXP_OPTIM
static QCache<QRegExp3Engine> *engineCache = 0;
#endif

static QRegExp3Engine *newEngine( const QString& pattern, bool caseSensitive )
{
#ifndef QT_NO_REGEXP_OPTIM
    if ( engineCache != 0 ) {
	QRegExp3Engine *eng = engineCache->take( pattern );
...
#endif
    return new QRegExp3Engine( pattern, caseSensitive );
}

static void derefEngine( QRegExp3Engine *eng, const QString& pattern )
{
    if ( eng != 0 && eng->deref() ) {
#ifndef QT_NO_REGEXP_OPTIM
	if ( engineCache == 0 ) {
	    engineCache = new QCache<QRegExp3Engine>;
...
QRegExp3::QRegExp3()
{
    eng = new QRegExp3Engine( TRUE );
    priv = new QRegExp3Private;
...
/*!
  ...
  \sa search() searchRev() QRegExp3Validator
*/
