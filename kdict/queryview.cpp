...
SaveHelper::SaveHelper(QString saveName, QString filter, QWidget *parent)
...
QFile* SaveHelper::getFile(QString dialogTitle)
{
...
BrowseData::BrowseData(char *Nhtml,QString NqueryText)
...
: QVBox( _parent), actBack(0L), actForward(0L), actQueryCombo(0L), browsePos(0), isRendering(false)
{
  ...
  connect(interface,SIGNAL(resultReady(char* &)),this,SLOT(resultReady(char* &)));
}
...
bool QueryView::browseBackPossible()
{
  return (browsePos > 0)? true:false;
}


bool QueryView::browseForwardPossible()
{
...
void QueryView::optionsChanged()
{
  ...
  currentHTMLHeader += QString("p.heading {  margin-left:0mm; margin-top:2mm; margin-bottom:2mm; padding:1mm; background-color:%1; color:%2; font-family:%3; font-size:%4px; ").arg(global->headingsBackgroundColor().name()).arg(global->headingsTextColor().name()).arg(global->headingsFont().family()).arg(global->headingsFont().pointSize());
  ...
  currentHTMLHeader += QString("pre { font-family:%1; font-size:%2px; ").arg(global->textFont().family()).arg(global->textFont().pointSize());
  ...
  part->setCharset(KGlobal::charsets()->name(global->charset),true);
...
void QueryView::saveQuery()
{
  if (!browseList.isEmpty()) {
    ...
    if (file) {
      ...
      stream << currentHTMLHeader+QString::fromUtf8(brw->html);
    }
  }
}
...
void QueryView::showResult()
{
  ...
  if (browseList.isEmpty()) {
    ...
    part->write(currentHTMLHeader+QString::fromUtf8(brw->html));
    part->end();
    part->view()->setFocus();
  }
}
...
void QueryView::resultReady(char* &result)
{
