...
JobData::JobData(QueryType Ntype,bool NnewServer,QString const& Nserver,int Nport,
  ...
  : type(Ntype), error(ErrNoErr), canceled(false), numFetched(0), result(0L),
    matches(0L), defines(0L), newServer(NnewServer),server(Nserver), port(Nport),
    timeout(Ntimeout), pipeSize(NpipeSize), idleHold(NidleHold), authEnabled(NAuthEnabled),
...
: job(0L), resultSizeInc(10000), inputSize(10000), fdPipeIn(NfdPipeIn),
...
void DictAsyncClient::waitForWork()
{
  ...
  while (true) {
    ...
    if (job) {
      ...
      job->result = new char[resultSizeInc];
      job->result[0] = 0;
      resultPos = 0;
      resultSize = resultSizeInc;
...
void DictAsyncClient::define()
{
  ...
  job->defines = new QStrList;
  
  for (unsigned int i=0;i<job->databases.count();i++) {
    command = "define ";
    command += job->databases.at(i);
    command += " \"";
    command += job->query;
    command += "\"\r\n";
  ...
  if (job->numFetched == 0) {
    ...
    if (job->numFetched == 0) {
      resultAppend("<body>\n<p class=\"heading\">\n");
      resultAppend((i18n("No definitions found for \'%1'!").arg(QString::fromUtf8(job->query))).utf8().data());
      resultAppend("</p>\n</html></body>");
    } else {
      // html header...
      resultAppend("<body>\n<p class=\"heading\">\n");
      resultAppend((i18n("No definitions found for \'%1\', perhaps you mean:").arg(QString::fromUtf8(job->query))).utf8().data());
      resultAppend("</p>\n<table width=\"100%\" cols=2>\n");

      QCString lastDb;
      do {
        char *s = strchr(job->matches->getFirst(),' ');
        if (s) {
          s[0] = 0;
          if (lastDb != job->matches->getFirst()) {
            if (lastDb.length())
              resultAppend("</pre></td></tr>\n");
            lastDb = job->matches->getFirst();
          ...
          if (strlen(s)>2) {
            ...
            resultAppend(s,strlen(s)-1);
            resultAppend("\">");
            resultAppend(s,strlen(s)-1);
            resultAppend("</a> ");
          }
        }
        job->matches->removeFirst();
      } while (job->matches->count());
      resultAppend("\n</pre></td></tr></table>\n</body></html>");
      job->numFetched = 0;
    }
  }
...
QCString htmlString(const QCString &raw)
{
...
QCString generateDefineLink(const QCString &raw)
{
  QRegExp http("http://[^\\s<>()\"|\\[\\]{}]+");
  QRegExp ftp("ftp://[^\\s<>()\"|\\[\\]{}]+");
  ...
  matchPos = http.match(raw, 0, &matchLen);
  if (-1 != matchPos) {
    httpMatch = true;
  } else {
    matchPos = ftp.match(raw, 0, &matchLen);
    httpMatch = false;
  }
...
bool DictAsyncClient::getDefinitions()
{
  ...
  while (job->defines->count()>0) {
    ...
    cmdBuffer = "";
    do {
      cmdBuffer += job->defines->at(0);
      defCount++;
      job->defines->removeFirst();
        ...
        while (!defineDone) {
          ...
          case 151: {      // define: 151 word database name - text follows
            ...
            while (!bodyDone) {
              ...
              if (!bodyDone) {
                ...
                if (!bracketBuff.isEmpty()) {
                  s = strchr(line,'}');
                  if (!s)
                    resultAppend(bracketBuff);
                  else {
                    s[0] = 0;
                    bracketBuff.remove(0,1);       // remove '{'
                    bracketBuff += line;
                    line = s+1;
                    resultAppend(generateDefineLink(bracketBuff));
                  }
                  bracketBuff = "";
                }
                s = strchr(line,'{');
                while (s) {
                  resultAppend(line,s-line);
                  line = s;
                  s = strchr(line,'}');
                  if (s) {
                    s[0] = 0;
                    line++;
                    resultAppend(generateDefineLink(line));
                    line = s+1;
                    s = strchr(line,'{');
                  } else {
                    bracketBuff = line;
            ...
            if (hashList.find(context.hexDigest())>=0)         // duplicate??
              resultPos = oldResPos;              // delete the whole definition
            else {
...
{
  job->matches = new QStrList();
  int todo = job->databases.count();
  int response;
  cmdBuffer = "";
  ...
  while (todo) {
    ...
    do {
      cmdBuffer += "match ";
      cmdBuffer += job->databases.at(job->databases.count()-todo);
      cmdBuffer += " ";
      cmdBuffer += job->strategy;
      cmdBuffer += " \"";
      cmdBuffer += job->query;
      cmdBuffer += "\"\r\n";
      send++;
      todo--;
    } while ((todo>0)&&((int)cmdBuffer.length()<job->pipeSize));
...
void DictAsyncClient::showDatabases()
{
  ...
  // html header...
  resultAppend("<body>\n<p class=\"heading\">\n");
  resultAppend(i18n("Available Databases:").utf8().data());
  resultAppend("\n</p>\n<table width=\"100%\" cols=2>\n");
...
void DictAsyncClient::showDbInfo()
{
  ...
  // html header...
  resultAppend("<body>\n<p class=\"heading\">\n");
  resultAppend((i18n("Database Information [%1]:").arg(QString::fromUtf8(job->query))).utf8().data());
...
void DictAsyncClient::showStrategies()
{
  ...
  // html header...
  resultAppend("<body>\n<p class=\"heading\">\n");
  resultAppend(i18n("Available Strategies:").utf8().data());
  resultAppend("\n</p>\n<table width=\"100%\" cols=2>\n");
...
void DictAsyncClient::showInfo()
{
  ...
  // html header...
  resultAppend("<body>\n<p class=\"heading\">\n");
  resultAppend(i18n("Server Information:").utf8().data());
  resultAppend("\n</p>\n<pre><p class=\"definition\">\n");
...
void DictAsyncClient::openConnection()
{
  ...
  if (ks.connect() < 0) {
    if (ks.status() == IO_LookupError)
      ...
      resultPos = 0;
      resultAppend(KExtendedSocket::strError(ks.status(), errno).local8Bit().data());
...
bool DictAsyncClient::waitForRead()
{
  ...
  if (ret > 0) {
    ...
    if (FD_ISSET(tcpSocket,&fdsE)||FD_ISSET(fdPipeIn,&fdsE)) {  // broken pipe, etc
      if (job) {
        resultPos = 0;
        resultAppend(i18n("The connection is broken.").utf8().data());
        job->error = JobData::ErrCommunication;
      }
      closeSocket();
      return false;
...
{
  resultAppend(str,strlen(str));
}


void DictAsyncClient::resultAppend(const char* str,unsigned int length)
{
  if (length >= resultSizeInc)  // just in case, should never happen
    length = resultSizeInc-1;
  if (resultPos+length>=resultSize) {    // we must expand the result string
    resultSize += resultSizeInc;
    char *newresult = new char[resultSize];
    memcpy(newresult,job->result,resultPos);
    delete job->result;
    job->result = newresult;
  }
  memcpy(&job->result[resultPos],str,length);
  resultPos += length;
  job->result[resultPos] = 0;        // terminate string;
}


void DictAsyncClient::resultAppend(char chr)
{
  if (resultPos+1>=resultSize) {    // we must expand the result string
    resultSize += resultSizeInc;
    char *newresult = new char[resultSize];
    memcpy(newresult,job->result,resultPos);
    delete job->result;
...
void DictInterface::define(QCString query)
{
...
void DictInterface::getDefinitions(QStrList* &query)
{
  JobData *newjob = new JobData(JobData::TGetDefinitions,newServer,global->server,global->port,
                                global->idleHold,global->timeout,global->pipeSize,global->authEnabled,
...
void DictInterface::match(QCString query)
{
...
void DictInterface::showDbInfo(QCString db)
{
  db = db.simplifyWhiteSpace();   // cleanup query string
  if (db.isEmpty())
    return;
  if (db.length()>100)               // shorten if necessary
    db.truncate(100);
  JobData *newjob = new JobData(JobData::TShowDbInfo,newServer,global->server,global->port,
                                global->idleHold,global->timeout,global->pipeSize,global->authEnabled,
                                global->user,global->secret,global->headLayout);
  newServer = false;
  newjob->query = db.copy();            // construct job...
  insertJob(newjob);
}
  ...
  if (!job->canceled) {  // non-interupted job?
    if (JobData::ErrNoErr == job->error) {
      ...
      case JobData::TUpdate :
        ...
        const char * s;
        for ( QStrListIterator it(job->databases); (s=it.current()); ++it )
          global->serverDatabases.append(QString::fromUtf8(s));
        ...
        for ( QStrListIterator it(job->strategies); (s=it.current()); ++it )
          global->strategies.append(QString::fromUtf8(s));
      ...
      case JobData::ErrBadHost:
        errMsg = i18n("Unable to connect to:\n%1:%2\n\nCan't resolve hostname.").arg(job->server).arg(job->port);
        break;
      ...
      case JobData::ErrServerError:
        errMsg = i18n("The server sent an unexpected reply:\n\"%1\"\nThis shouldn't happen, please consider\nwriting a bug report").arg(QString::fromUtf8(job->result));
        break;
...
JobData* DictInterface::generateQuery(JobData::QueryType type,QCString query)
{
  ...
  if (query.length()>300)               // shorten if necessary
    query.truncate(300);
  unsigned int i(0);
  while (i < query.length()) {          // remove remaining illegal chars...
    char c = query.data()[i];
    if ((c=='\"')||(c=='\\'))
      query.remove(i,1);
    else
  ...
  JobData *newjob = new JobData(type,newServer,global->server,global->port,
                                ...
                                global->user,global->secret,global->headLayout);
  newServer = false;
  newjob->query = query.copy();            // construct job...
