...

{
  ...
  time.sprintf("[%02d:%02d]", now.hour(), now.minute());
...
void ircListItem::paint(QPainter *p)
{
  ...
  if(need_update == TRUE){
    ...
    if(dbuffer == 0x0){
      kdDebug() << "ircListItem::paint() Failed to get a paint pixmap!! Giving up" << endl;
      need_update = TRUE;
    }
    else
      need_update = FALSE;
...
void ircListItem::setupPainterText()
{
  ...
  if(dbuffer == 0x0){
    ...
    if(PaintCache->insert(CacheId, dbuffer, 1) == false)
      kdDebug() << "Insert into cache failed" << endl;
  ...
  if ( kSircConfig->timestamp) {
      //we have timestamp, need its xsize
      timestamp_length = fm.width(time.latin1()) + 5;
      xPos += timestamp_length;
  }
  ...
  paint_text->clear();
  int max_width = parent_lb->viewport()->width() - 35 - timestamp_length;
  int realIndex = 0;
  ...
  if((old_width != width(0)) ||
    ...
    dbuffer->fill(parent_lb->colorGroup().base());
    forceClear = FALSE;
  }
  ...
  if((dbuffer->isNull() == FALSE) &&
    ...
    p.setPen(*colour);
    p.setBackgroundColor(parent_lb->colorGroup().base());

    if ( kSircConfig->timestamp ) {
        // Draw the time stamp.
        KSPainter::colourDrawText(&p, 1, yPos, time.latin1());
    }

    if(pm){
      p.drawPixmap(xPos,0,*pm);
      xPos += pm->width() + 5;
    }
    ...
    kdDebug() << "Start failed!\n";
    if(dbuffer->isNull())
      kdDebug() << "dbuffer is NULL!!!!!!\n";
    dbuffer->resize(1,1);
    need_update = TRUE;
  }
