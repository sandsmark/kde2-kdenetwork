...
void PukeController::Writeable(int fd) 
{
  if(qidConnectFd[fd]){
    ...
    kdDebug() << "PUKE: Unkonwn fd: " << fd << endl;
  }
}
...
void PukeController::writeBuffer(int fd, PukeMessage *message) 
{
  if(qidConnectFd[fd]){
    ...
    //      kdDebug() << "PUKE: Writing to FD that's not writeable: " << fd << endl;
      //    }
    if(message != 0){
      int bytes = 0;
...
//	  perror("Puke: write on socket failed");
    ...
    closefd(fd);
    kdDebug() << "PUKE: Attempt to write to unkown fd:" << fd << endl;
  }
}
...
void PukeController::Traffic(int fd) 
{
  ...
  while((bytes = read(fd, &pm, 5*sizeof(int))) > 0){
    if(bytes != 5*sizeof(int)){
      kdDebug() << "Short message, Got: " << bytes << " Wanted: " << sizeof(PukeMessage) << " NULL Padded" << endl;
    }
#ifdef DEBUG
...
void PukeController::initHdlr() 
{
  ...
  wc = new widgetCreate;
  wc->wc = PKSircListBox::createWidget;
  widgetCF.insert(PWIDGET_KSIRCLISTBOX, wc);
...
void PukeController::hdlrPukeDeleteWidget(int fd, PukeMessage *pm) 
{
  ...
  if(qidWS == 0){
    kdDebug() << "WidgetRunner:: no such set of widget descriptors?" << endl;
...
void PukeController::closefd(int fd) 
{
  ...
  if(qidConnectFd[fd] == NULL){
    kdDebug() << "PukeController: Connect table NULL, closed twice?" << endl;
    return;
  }
  ...
  QIntDict<WidgetS> *qidWS = WidgetList[fd];
  if(qidWS == 0){
    kdDebug() << "WidgetRunner:: Close called twice?" << endl;
    bClosing = FALSE;
    return;
  }
  ...
  do {
    ...
    if(it.count() == 0){
      kdDebug() << "WidgetRunner: nothing left to delete\n" << endl;
      break;
    }
...
void PukeController::pobjectDestroyed(){
  ...
  if(checkWidgetId(pwi) == TRUE){
    ...
    kdDebug() << "Someone stole pwi: " << pwi->fd << ", " << pwi->iWinId << endl;
  }
