...
PWidget::PWidget(PObject *)
  : PObject()
{
  //  kdDebug() << "PWidget constructor called" << endl;
...
PWidget::~PWidget()
{
  //  kdDebug() << "PWidget: in destructor" << endl;
  /*
  delete widget();
  w = 0;
  setWidget(0);
...
void PWidget::messageHandler(int fd, PukeMessage *pm)
{
  ...
  case PUKE_WIDGET_MOVE:
	...
	throw(errorCommandFailed(PUKE_INVALID,10));
      ...
      //      kdDebug() << "Moving to: " << pm->iArg << " => " << pos[0] << " " << pos[1] << endl;
      widget()->move(x, y);
...
{
  //  kdDebug() << "PWidget widget called" << endl;
  return w;
}
...
void PWidget::eventNone(QObject *, QEvent *e)  
{
  ...
  //  kdDebug() << "PWidget: eventNone" << endl;
...
void PWidget::eventFocus(QObject *, QEvent *e)  
{
  ...
  //  kdDebug() << "PWidget: eventFocus" << endl;
