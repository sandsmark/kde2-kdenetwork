/**********************************************************************
 ...
 $$Id: ioDCC.cpp 77798 2001-01-13 12:48:56Z hausmann $$
...
void KSircIODCC::sirc_receive(QString str, bool )
{
  ...
  if(str.find("DCC GET read:", 0) != -1){
    ...
    int pos2 = str.find(")", pos1);
    if((pos1 < 0) || (pos2 < 0)){
      kdDebug() << "FILENAME Pos1/Pos2 " << pos1 << '/' << pos2 << endl;
      return;
    }
    QString filename = str.mid(pos1, pos2 - pos1);
    ...
    pos2 = str.find(")", pos1);
    if((pos1 < 0) || (pos2 < 0)){
      kdDebug() << "SIZE Pos1/Pos2 " << pos1 << '/' << pos2 << endl;
      return;
    }
    ...
    pos2 = str.find(" ", pos1);
    if((pos1 < 0) || (pos2 < 0)){
      kdDebug() << "FROM Pos1/Pos2 " << pos1 << '/' << pos2 << endl;
      return;
    }
    ...
    int pos2 = str.find(" ", pos1);
    if((pos1 < 0) || (pos2 < 0)){
      kdDebug() << "NICK Pos1/Pos2 " << pos1 << '/' << pos2 << endl;
      return;
    }
    QString nick = str.mid(pos1, pos2 - pos1);
    // Find the filename
